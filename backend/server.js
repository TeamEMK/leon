...
async function _ensureAppState() {
  // updated_at ka auto-touch dono dialect me alag tarah se hota hai:
  //   MySQL    — column par hi ON UPDATE CURRENT_TIMESTAMP
  //   Postgres — trigger, kyunki wahan ON UPDATE CURRENT_TIMESTAMP hota hi nahi
  //
  // Pehle yahan sirf Postgres wala DO $$…$$ block tha, bina dialect dekhe. MySQL
  // par `pg_proc` jaisi Postgres-only catalog table par wo syntax error deta tha,
  // aur ye function wahin gir jaata tha — yaani caller ko lagta "app_state
  // unavailable" aur WhatsApp scheduler kabhi start hi nahi hota. Postgres par
  // ye kabhi dikhta nahi tha, isliye pakda nahi gaya; par "New Client Copy" me
  // MySQL ek option hai, to jis client ne MySQL chuna uske yahan WhatsApp
  // automation chup-chaap band pada rehta.
  const isMysql = db.kind === 'mysql';
  await db.query(`CREATE TABLE IF NOT EXISTS app_state (
    k VARCHAR(64) NOT NULL PRIMARY KEY,
    v VARCHAR(255) DEFAULT NULL,
    updated_at TIMESTAMP NOT NULL DEFAULT NOW()${isMysql ? ' ON UPDATE CURRENT_TIMESTAMP' : ''}
  )`);
  if (isMysql) return;                 // upar column me hi ho gaya — trigger ki zarurat nahi

  // Trigger function schema migration me banta hai; yahan sirf tab lagate hain
  // jab wo maujood ho, taaki fresh DB par ye bootstrap bina migration ke bhi na gire.
  await db.query(`
    DO $$
    BEGIN
...
const {
  ...
  capExtraRowNames: _capExtraRowNames, healExtraRows: _healExtraRows,
  getSheetGid: _getSheetGid, forceCellNumber: _forceCellNumber,
} = require('./lib/sheet-cols');
const { detectSteps: _detectSteps } = require('./lib/fms-detect');
...
async function _ensureFmsSchema() {
  ...
  // Header ka NAAM — sheet me column khiskne par letter isi se theek hota hai
  // (migration 005 wahi karti hai; yahan boot par bhi taaki purane deploy na atkein).
  await addCol(`ALTER TABLE fms_extra_rows ADD COLUMN IF NOT EXISTS col_name VARCHAR(255)`);
...
app.get('/api/fms/:id', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    // Steps aur extra inputs bhi NAAM se heal karke bhejte hain. Sirf display ki
    // baat nahi hai: editor jo letter dikhata hai wahi wapas save hota hai, aur
    // save par header-naam usi letter se capture hota hai (_capStepNames). Stale
    // letter bhejte to admin ke ek baar "Save" dabate hi GALAT naam pakka ho
    // jaata — aur tab heal bhi usi galat column par le jaata.
    try {
      const fmsHdrs = await _fetchSheetHeaders(sheets[0]);
      if (fmsHdrs.length) for (const step of steps) {
        _healStepCols(step, fmsHdrs);
        _healExtraRows(step.extraRows || [], fmsHdrs);
        try { step.show_cols_parsed = JSON.parse(step.show_cols || '[]'); } catch(e) {}
      }
    } catch (e) { console.error('fms GET step heal skipped:', e.message); }
...
app.post('/api/fms', requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      // col_name bhi save hota hai: sheet me beech me column jud jaye to letter
      // isi naam se theek ho jaata hai (lib/sheet-cols.js ka healExtraRows).
      // headers upar _capStepNames ke liye pehle hi padhe ja chuke hain.
      if (s.extraInput==='yes' && s.extraRows?.length) for (const row of s.extraRows) await conn.query('INSERT INTO fms_extra_rows (step_id,row_label,col_letter,col_name,field_type,dropdown_options,required) VALUES (?,?,?,?,?,?,?)', [stepId, row.label||row.col_letter||'', row.col_letter||'', _intakeNameAt(headers, row.col_letter||'') || null, row.field_type||'text', row.dropdown_options||'', [0,1,2].includes(Number(row.required))?Number(row.required):(row.required===false?0:1)]);
    }
    await conn.commit();
...
// Sheet padh kar poori step config bana do — "New FMS" me har step haath se
// bharne ki jagah. Nateeja sirf ek SUJHAAV hai: admin isi screen par sudhaar
// kar ke save karta hai, isliye jahan shak ho wahan khaali chhodte hain.
//
// fetch-headers se alag isliye hai ki yahan cells ka formatting bhi chahiye —
// data validation (dropdown ke options), formula (wo column chhodne ke liye)
// aur number format (date/number pehchanne ke liye). Ye sab values.get nahi
// deta, iske liye spreadsheets.get + includeGridData chahiye.
app.post('/api/fms/analyze-sheet', requireAuth, requireAdmin, async (req, res) => {
  try {
    const { sheetId, sheetName, headerRow } = req.body;
    if (!sheetId) return res.status(400).json({ error: 'sheetId required' });
    const sheetsApi = await getSheetsClient(['https://www.googleapis.com/auth/spreadsheets.readonly']);
    const spreadsheetId = extractSpreadsheetId(sheetId);
    const hRow = parseInt(headerRow) || 1;
    const tab = (sheetName || '').trim();
    const qTab = !tab ? '' : (/^[A-Za-z0-9_]+$/.test(tab) ? tab : `'${tab.replace(/'/g, "''")}'`);

    // Header ke UPAR wali row bhi chahiye — step ka naam aksar wahin merged
    // cell me hota hai. Neeche 20 data rows kaafi hain: type pehchanne ke liye
    // itne hi bahut hain, aur poori sheet padhna 10,000-row sheet par bhaari.
    const SAMPLE = 20;
    const topRow = Math.max(1, hRow - 1);
    const lastRow = hRow + SAMPLE;
    const range = qTab ? `${qTab}!${topRow}:${lastRow}` : `${topRow}:${lastRow}`;
    const resp = await sheetsApi.spreadsheets.get({
      spreadsheetId, ranges: [range], includeGridData: true,
      fields: 'sheets(data(rowData(values(formattedValue,userEnteredValue,dataValidation,effectiveFormat(numberFormat)))))',
    });
    const rowData = (((resp.data.sheets || [])[0] || {}).data || [{}])[0].rowData || [];
    const hOff = hRow - topRow;                       // header row iss slice me kahan hai
    const cellsOf = r => ((rowData[r] || {}).values) || [];
    const textOf = cells => cells.map(v => (v && v.formattedValue != null) ? String(v.formattedValue) : '');

    const headers = textOf(cellsOf(hOff));
    const groupRow = hOff > 0 ? textOf(cellsOf(hOff - 1)) : [];

    // Har column ke kuch data cells — sirf wahi jaankari jo detect ko chahiye.
    const sampleCells = {};
    for (let r = hOff + 1; r < rowData.length; r++) {
      const vals = cellsOf(r);
      for (let c = 0; c < vals.length; c++) {
        const v = vals[c] || {};
        const cond = v.dataValidation && v.dataValidation.condition;
        let validation = null;
        if (cond) {
          const listVals = (cond.values || []).map(x => x && x.userEnteredValue).filter(x => x != null);
          validation = { type: cond.type, values: cond.type === 'ONE_OF_LIST' ? listVals : [], ref: cond.type === 'ONE_OF_RANGE' ? listVals[0] : null };
        }
        const cell = {
          value: v.formattedValue == null ? '' : String(v.formattedValue),
          formula: !!(v.userEnteredValue && v.userEnteredValue.formulaValue),
          validation,
          numberFormat: (v.effectiveFormat && v.effectiveFormat.numberFormat && v.effectiveFormat.numberFormat.type) || null,
        };
        if (cell.value === '' && !cell.formula && !cell.validation) continue;   // khaali cell bekaar hai
        (sampleCells[c] = sampleCells[c] || []).push(cell);
      }
    }

    // Dropdown ka "list from a range" — options doosri jagah pade hain, unhe
    // padh kar bhar dete hain, warna admin ko wahi list haath se likhni padti.
    const refs = new Map();
    for (const c of Object.keys(sampleCells)) {
      const hit = sampleCells[c].find(x => x.validation && x.validation.ref);
      if (hit) refs.set(c, String(hit.validation.ref).replace(/^=/, ''));
    }
    if (refs.size) {
      try {
        const list = [...refs.values()];
        const got = await sheetsApi.spreadsheets.values.batchGet({ spreadsheetId, ranges: list });
        const byRange = new Map(list.map((r, i) => [r, ((got.data.valueRanges || [])[i] || {}).values || []]));
        for (const [c, r] of refs) {
          const opts = (byRange.get(r) || []).map(row => String((row || [])[0] == null ? '' : row[0]).trim()).filter(Boolean);
          if (opts.length) for (const cell of sampleCells[c]) if (cell.validation) { cell.validation.values = opts; cell.validation.type = 'ONE_OF_LIST'; }
        }
      } catch (e) { console.error('  ⚠️  FMS dropdown range read skipped:', e.message); }
    }

    const [users] = await db.query('SELECT id, name FROM users ORDER BY name ASC');
    const steps = _detectSteps({ headers, groupRow, sampleCells, users });

    // UI column LETTER par chalta hai, detect index par — yahan badal dete hain.
    const out = steps.map(s => ({
      stepName: s.stepName,
      planCol: s.planCol == null ? '' : idxToCol(s.planCol),
      actualCol: s.actualCol == null ? '' : idxToCol(s.actualCol),
      doerNameCol: s.doerNameCol == null ? '' : idxToCol(s.doerNameCol),
      doers: s.doers,
      extraInput: s.extraRows.length ? 'yes' : 'no',
      extraRows: s.extraRows.map(r => ({
        label: r.label, col_letter: idxToCol(r.col_index),
        field_type: r.field_type, dropdown_options: r.dropdown_options, required: r.required,
      })),
    }));
    res.json({ steps: out, headerCount: headers.filter(h => String(h).trim()).length });
  } catch (err) {
    if (err.code === 403) return res.status(400).json({ error: 'Access denied. Share sheet with service account.' });
    if (err.code === 404) return res.status(400).json({ error: 'Sheet not found. Check Sheet ID.' });
    console.error(err); res.status(500).json({ error: 'Server error. Please try again.' });
...
app.get('/api/fms-tasks/:id', requireAuth, async (req, res) => {
  try {
    ...
    // Yahan se doer ka UI banta hai, aur values /rows se aati hain — jo columns
    // naam se heal karke bhejta hai. Dono ek jaise hone chahiye, warna input to
    // dikhta hai par uski maujooda value nahi milti (lookup letter par hota hai).
    try {
      const fmsHdrs = await _fetchSheetHeaders(sheets[0]);
      if (fmsHdrs.length) for (const step of steps) {
        _healStepCols(step, fmsHdrs);
    ...
    } catch (e) { console.error('fms-tasks GET step heal skipped:', e.message); }
    res.json({ sheet: sheets[0], steps });
  } catch (err) { console.error(err); res.status(500).json({ error: 'Server error. Please try again.' }); }
});
...
app.get('/api/fms-tasks/:fmsId/steps/:stepId/rows', requireAuth, async (req, res) => {
  try {
    ...
    // Actual column set hi na ho to "ho chuka" aur "baaki hai" me farak karne
    // ka koi zariya nahi bachta: actualIdx -1 rehta hai, har row ki actual
    // value khaali padhi jaati hai, aur HAR planned row pending dikhne lagti
    // hai — chahe sheet me wo kab ki poori ho chuki ho. Doer use dobara "Done"
    // karne chala jaata hai aur wahan jaakar error milta hai.
    //
    // Jhooti list dene se behtar hai saaf bata dena ki config adhuri hai.
    if (!String(step.actual_col || '').trim()) {
      return res.status(400).json({
        error: `"${step.step_name}" has no Actual column set, so rows that are already done cannot be told apart from pending ones. Set it in FMS Admin.`
      });
    }
    const planIdx = colToIdx(step.plan_col);
    const actualIdx = colToIdx(step.actual_col);
...
app.get('/api/fms-tasks/:fmsId/steps/:stepId/done-rows', requireAuth, async (req, res) => {
  try {
    ...
    // extraCols headers padhne ke BAAD banti hai (neeche) — pehle yahan banti thi,
    // yaani heal se pehle, aur sheet me column judne par galat cells padhe jaate the.
    ...
    const headers = allRows[headerRowIdx] || [];
    _healStepCols(step, headers);
    _healExtraRows(extraRows, headers);   // extra inputs bhi naam se — letter shift-safe
...
app.post('/api/fms-tasks/:fmsId/steps/:stepId/update-extra', requireAuth, async (req, res) => {
  try {
    ...
    // Column browser ke bheje letter se NAHI, apne config se tay hota hai.
    // Wajah: page khulne ke baad agar sheet me koi column jud gaya, to browser
    // ke paas purana letter reh jaata hai aur value paas wale column me chali
    // jaati thi — chupchaap, kyunki galat column par likhna bhi "success" hai.
    // Ab step ke extra rows DB se laate hain, unhe header ke naam se current
    // position par heal karte hain, aur client ki value sirf row ki `id` se
    // milate hain.
    const [defs] = await db.query('SELECT * FROM fms_extra_rows WHERE step_id=? ORDER BY id ASC', [req.params.stepId]);
    const hdrs = await _fetchSheetHeaders(sheet);
    _healExtraRows(defs, hdrs);
    const byId = new Map(defs.map(d => [String(d.id), d]));

    const batchData = [];
    for (const ei of extraInputs) {
      if (!ei) continue;
      // Purana browser (cached JS) sirf colLetter bhejta hai — us haal me letter
      // hi use karte hain, warna bina naye page reload ke update ruk jaata.
      const def = ei.id != null ? byId.get(String(ei.id)) : null;
      const col = def ? def.col_letter : ei.colLetter;
      if (!col) continue;
      batchData.push({ range: `${qTab}!${String(col).toUpperCase()}${rowNumber}`, values: [[ei.value == null ? '' : ei.value]] });
    }
...
app.post('/api/fms-tasks/:fmsId/steps/:stepId/done', requireAuth, async (req, res) => {
  try {
    ...
    // Step ke apne columns upar _healStepCols se theek ho chuke hain, par extra
    // inputs pehle browser ke bheje letter par likhte the — yaani sheet me column
    // judte hi ye value paas wale column me chali jaati thi. Ab inhe bhi apne
    // config se, header ke naam se resolve karte hain (upar wale hdrs se, koi
    // nayi API call nahi).
    if (extraInputs && extraInputs.length) {
      const [defs] = await db.query('SELECT * FROM fms_extra_rows WHERE step_id=? ORDER BY id ASC', [step.id]);
      _healExtraRows(defs, hdrs);
      const byId = new Map(defs.map(d => [String(d.id), d]));
      for (const ei of extraInputs) {
        if (!ei || ei.value === undefined || ei.value === '') continue;
        // Purana browser sirf colLetter bhejta hai — tab wahi chalega.
        const def = ei.id != null ? byId.get(String(ei.id)) : null;
        const col = def ? def.col_letter : ei.colLetter;
        if (!col) continue;
        batchData.push({ range: `${qTab}!${String(col).toUpperCase()}${rowNumber}`, values: [[ei.value]] });
      }
    }
