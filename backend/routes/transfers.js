...
module.exports = function registerTransfersRoutes(app, ctx) {
  ...
  // List aur badge ka count EK HI FROM par chalte hain.
  //
  // Pehle count seedha task_transfers par chalta tha, jabki list users ke saath
  // teen alag INNER JOIN karti hai (from_user, to_user, requested_by). Jis row
  // ka koi bhi user delete ho chuka ho, wo count me aa jaati thi par list me
  // nahi — badge par number aur andar khaali. Ek hi jagah likhne se ab dono
  // alag nahi ho sakte.
  const TRANSFERS_FROM = `FROM task_transfers tt
  ...
  app.get('/api/transfers/count', requireAuth, requireAdminOrHod, async (req, res) => {
    try {
      ...
      const role = req.session.role;
      let count = 0;
      // Yahi joins upar wali list me bhi hain. Inke bina badge un rows ko bhi
      // gin leta tha jinka koi user delete ho chuka ho — list unhe INNER JOIN
      // par chhod deti hai, to badge par number dikhta tha aur andar khaali.
      if (role === 'admin') {
        const [r] = await db.query(`SELECT COUNT(*) AS c ${TRANSFERS_FROM} WHERE tt.status='pending'`);
        count = r[0].c;
      } else {
        const [me] = await db.query('SELECT department FROM users WHERE id=?', [uid]);
        const dept = me[0]?.department || '';
        ...
        if (deptUsers.length) {
          const ids = deptUsers.map(u=>u.id);
          const [r] = await db.query(`SELECT COUNT(*) AS c ${TRANSFERS_FROM} WHERE tt.status='pending' AND (tt.from_user IN (${ids.map(()=>'?').join(',')}) OR tt.to_user IN (${ids.map(()=>'?').join(',')}))`, [...ids,...ids]);
          count = r[0].c;
        }
      }
      res.json({ count });
