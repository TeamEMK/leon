...
// ── Extra-input rows: letters ko naam se resilient banao (steps jaisa) ──
// fms_extra_rows ki har row sheet ke ek column par likhti hai. Yahan pehle sirf
// col_letter tha — naam nahi. Isliye sheet me beech me column jodte hi saare
// extra inputs ek jagah khisak jaate the: padhne me galat value dikhti thi, aur
// LIKHNE me (step Done karte waqt ya row ke fields update karte waqt) data
// doosre column me chala jaata tha. Likhne wali galti chupi rehti thi — error
// koi nahi aata, bas paas wale column ki value dab jaati thi.
//
// `near` ke liye row ka apna stored letter dete hain: "Planned"/"Actual" jaise
// naam har step me dobara aate hain, aur stored letter ke sabse nazdeek wala
// match hi us step ka apna column hota hai.
function _capExtraRowNames(rows, headers) {
  if (!Array.isArray(rows) || !Array.isArray(headers) || !headers.length) return rows;
  for (const r of rows) {
    if (!r) continue;
    const letter = r.col_letter || r.colLetter;
    if (letter) r.col_name = _intakeNameAt(headers, letter) || null;
  }
  return rows;
}
function _healExtraRows(rows, headers) {
  if (!Array.isArray(rows) || !Array.isArray(headers) || !headers.length) return rows;
  for (const r of rows) {
    if (!r || !r.col_name) continue;     // purana config (naam nahi) -> letter waisa hi
    const near = colToIdx(r.col_letter || '');
    const i = _findColByNameNear(headers, r.col_name, near);
    if (i >= 0) r.col_letter = idxToCol(i);
  }
  return rows;
}
