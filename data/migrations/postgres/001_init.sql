-- Postgres schema — MySQL dump se convert kiya gaya.
-- enum -> text + CHECK, tinyint -> smallint (0/1 wahi rehta hai, boolean nahi),
-- KEY -> CREATE INDEX (naam table-prefixed, kyunki Postgres me index namespace global hai),
-- ON UPDATE CURRENT_TIMESTAMP -> BEFORE UPDATE trigger.

-- MySQL ke ON UPDATE CURRENT_TIMESTAMP ka Postgres badal
...
CREATE TABLE payment_requests (
  ...
  payment_done_at timestamp NULL DEFAULT NULL,
...
CREATE TRIGGER app_state_touch BEFORE UPDATE ON app_state
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE TRIGGER hrm_candidates_touch BEFORE UPDATE ON hrm_candidates
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
ALTER TABLE week_plans ADD COLUMN updated_at timestamp NOT NULL DEFAULT NOW();
ALTER TABLE week_plans ADD CONSTRAINT week_plans_emp_week_uq UNIQUE (employee_id, start_date);
CREATE TRIGGER week_plans_touch BEFORE UPDATE ON week_plans
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();

-- Auto-increment counters (warna naye rows purane ids par takrayenge)
SELECT setval(pg_get_serial_sequence('cc_cards','id'), 40, false);
SELECT setval(pg_get_serial_sequence('cc_departments','id'), 15, false);
SELECT setval(pg_get_serial_sequence('cc_statements','id'), 40, false);
SELECT setval(pg_get_serial_sequence('cc_transactions','id'), 526, false);
SELECT setval(pg_get_serial_sequence('checklist_tasks','id'), 47158, false);
SELECT setval(pg_get_serial_sequence('daily_tasks','id'), 4834, false);
SELECT setval(pg_get_serial_sequence('delegation_tasks','id'), 8, false);
SELECT setval(pg_get_serial_sequence('fms_extra_rows','id'), 36, false);
SELECT setval(pg_get_serial_sequence('fms_sheets','id'), 6, false);
SELECT setval(pg_get_serial_sequence('fms_step_doers','id'), 90, false);
SELECT setval(pg_get_serial_sequence('fms_steps','id'), 59, false);
SELECT setval(pg_get_serial_sequence('holidays','id'), 3, false);
SELECT setval(pg_get_serial_sequence('hrm_candidates','id'), 4, false);
SELECT setval(pg_get_serial_sequence('hrm_message_log','id'), 115, false);
SELECT setval(pg_get_serial_sequence('leave_requests','id'), 193, false);
SELECT setval(pg_get_serial_sequence('meeting_attendees','id'), 75, false);
SELECT setval(pg_get_serial_sequence('meetings','id'), 150, false);
SELECT setval(pg_get_serial_sequence('message_logs','id'), 29, false);
SELECT setval(pg_get_serial_sequence('payment_requests','id'), 80, false);
SELECT setval(pg_get_serial_sequence('pr_cards','id'), 3217, false);
SELECT setval(pg_get_serial_sequence('queries','id'), 9, false);
SELECT setval(pg_get_serial_sequence('task_approvals','id'), 44, false);
SELECT setval(pg_get_serial_sequence('task_comments','id'), 5, false);
SELECT setval(pg_get_serial_sequence('task_subtasks','id'), 5, false);
SELECT setval(pg_get_serial_sequence('task_transfers','id'), 25, false);
SELECT setval(pg_get_serial_sequence('tasks','id'), 66, false);
SELECT setval(pg_get_serial_sequence('users','id'), 95, false);
SELECT setval(pg_get_serial_sequence('week_plans','id'), 248, false);
