...
// Sheet padh kar saare steps ek saath bhar do — naam, Planned/Actual columns,
// doer, aur har step ke extra inputs (dropdown ke options sheet ki apni data
// validation se). Server jahan pakka na ho wahan khaali chhodta hai, isliye
// yahan bhi khaali ko khaali hi rehne dete hain — admin isi screen par bhar
// lega. Purane steps replace hote hain, isliye pehle poochhte hain.
async function autoFillFMSSteps() {
  const sheetId = document.getElementById('fmsSheetId').value.trim();
  const sheetName = document.getElementById('fmsSheetName').value.trim();
  const headerRow = document.getElementById('fmsHeaderRow').value.trim();
  const note = document.getElementById('fmsAutoFillNote');
  const btn = document.getElementById('fmsAutoFillBtn');
  if (!sheetId) { showToast('Enter the Sheet ID first', 'error'); return; }
  if (fmsSteps.length && !confirm(`This replaces the ${fmsSteps.length} step(s) below with what the sheet says. Continue?`)) return;

  const label = btn.textContent;
  btn.disabled = true; btn.textContent = '⏳ Reading sheet…';
  note.style.display = 'none';
  try {
    const r = await api('/api/fms/analyze-sheet', 'POST', { sheetId, sheetName, headerRow });
    if (r.error) { showToast('⚠️ ' + r.error, 'error'); return; }
    if (!r.steps || !r.steps.length) {
      note.innerHTML = `Could not find any steps. This looks for a <b>Planned</b> column to start each step — if the sheet calls it something else, add the steps by hand.`;
      note.style.display = 'block';
      return;
    }
    // Headers bhi chahiye — step box ke column dropdown inhi se bharte hain.
    const hRes = await api('/api/fms/fetch-headers', 'POST', { sheetId, sheetName, headerRow });
    fmsSheetHeaders = (hRes && hRes.headers) || [];

    fmsSteps = r.steps.map(s => ({
      stepName: s.stepName || '', doers: s.doers || [],
      planCol: s.planCol || '', actualCol: s.actualCol || '',
      extraInput: s.extraInput || 'no', extraCol: '',
      extraRows: s.extraRows || [], showCols: [],
      delayReasonCol: '', doerNameCol: s.doerNameCol || '',
    }));
    const box = document.getElementById('fmsStepsContainer');
    box.innerHTML = '';
    fmsSteps.forEach((_, i) => appendFMSStepBox(i, 'fmsStepsContainer'));
    updateEditStepNav();

    // Jo khaali reh gaya wo saaf-saaf bata dete hain, taaki admin ko dhoondhna
    // na pade ki kya chhoot gaya.
    const noName = fmsSteps.filter(s => !s.stepName).length;
    const noDoer = fmsSteps.filter(s => !s.doers.length).length;
    const noActual = fmsSteps.filter(s => !s.actualCol).length;
    const extras = fmsSteps.reduce((n, s) => n + s.extraRows.length, 0);
    const gaps = [];
    if (noName)   gaps.push(`${noName} without a name`);
    if (noActual) gaps.push(`${noActual} without an Actual column`);
    if (noDoer)   gaps.push(`${noDoer} without a doer`);
    note.innerHTML = `Filled <b>${fmsSteps.length} step(s)</b> and <b>${extras} extra input(s)</b> from the sheet.` +
      (gaps.length ? ` Left blank on purpose: ${gaps.join(', ')} — a wrong guess is worse than an empty box, so please fill these in.` : '');
    note.style.display = 'block';
    showToast(`✅ ${fmsSteps.length} steps filled from the sheet`);
  } catch (e) {
    showToast('⚠️ Could not read the sheet', 'error');
  } finally {
  ...
  for (let di = 0; di < defs.length; di++) {
    ...
    if ((d.field_type || '') === 'file') {
      ...
      try {
        ...
        // id bhi jaati hai: server column apne config se tay karta hai, isliye
        // sheet me column judne par bhi value sahi jagah likhi jaati hai.
        extraInputs.push({ id: d.id, colLetter: cl, value: `=HYPERLINK("${data.url}","${fn}")` });
      } catch (e) {
      ...
      if (el) extraInputs.push({ id: d.id, colLetter: cl, value: el.value });
    }
  }
...
async function saveFMSDone() {
  ...
  // id ke saath — server column apne config se resolve karta hai (letter sirf
  // purane cached JS ke liye fallback hai).
  const extraInputs = extraRows.map((r, i) => {
    const el = document.getElementById(`fmsExtra_${i}`);
    return { id: r.id, colLetter: r.col_letter, value: el ? el.value.trim() : '' };
